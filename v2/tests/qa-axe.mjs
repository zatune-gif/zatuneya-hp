...
try {
  for (const profile of profiles) {
    ...
    try {
      ...
      const response = await page.goto(`${server.origin}/index.html`, { waitUntil: 'domcontentloaded' });
      assert.equal(response?.status(), 200, `axe ${profile.name}: index returns HTTP 200`);
      for (const scan of scans) {
        if (scan.settleMilliseconds > 0) await page.waitForTimeout(scan.settleMilliseconds);
        const results = await new AxeBuilder({ page }).withTags(wcagTags).analyze();
        console.log(`AXE ${profile.name} ${scan.name}: ${results.violations.length} WCAG 2/2.1 A/AA violations`);
        violations.push(...results.violations.map(violation => ({
          profile: `${profile.name} ${scan.name}`,
          violation
        })));
      }
    } finally {
...
console.log('PASS axe 0 WCAG 2/2.1 A/AA violations across immediate and settled mobile/desktop scans');
